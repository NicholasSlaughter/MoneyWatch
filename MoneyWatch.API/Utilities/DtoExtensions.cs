...
namespace MoneyWatch.API.Utilities
{
    ...
    {
        public static ApplicationUserDto AsDto(this ApplicationUser user) => new ApplicationUserDto(user.Id,
            user.First_Name,
            user.Last_Name,
            user.UserName,
            user.Email,
            ClaimsPrincipal.Current.Identities.First().Claims.ToList().FirstOrDefault(userClaim => userClaim.Type.Equals(user.UserName,StringComparison.OrdinalIgnoreCase)).Value,
            user.Expenses,
            user.Alerts);
